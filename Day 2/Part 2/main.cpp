...
bool isValidSet(std::string line){
    ...
    for(std::string str : all){
        ...
        for(int i = 0; i < 3; i++){
            if(cubeColor == color[i].first && nbreCube > color[i].second) isValide = false;
            
            if(cubeColor == color[0].first) minGreen = nbreCube > minGreen ? nbreCube : minGreen;
            else if(cubeColor == color[1].first) minBlue = nbreCube > minBlue ? nbreCube : minBlue;
            else if(cubeColor == color[2].first) minRed = nbreCube > minRed ? nbreCube : minRed;
        }
    }
